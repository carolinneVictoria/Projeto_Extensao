...
public function buscarProdutoCompra($idCompra, $idProduto) {
    $sql = "SELECT compraProduto.quantidade, compraProduto.valorUnitario, Produto.nomeProduto
            FROM compraProduto
            JOIN Produto ON compraProduto.idProduto = Produto.idProduto
            WHERE compraProduto.idCompra = ? AND compraProduto.idProduto = ?";
    $stmt = $this->conn->prepare($sql);
    $stmt->bind_param("ii", $idCompra, $idProduto);
    $stmt->execute();
